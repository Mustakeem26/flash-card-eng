  ...
  <motion.header :initial="{ opacity: 0, y: -20 }" :animate="{ opacity: 1, y: 0 }"
    ...
    <div class="max-w-5xl mx-auto flex justify-between items-center">
      ...
      <div class="flex items-center gap-5">
        <div
          class="w-18 h-14 bg-earth-800 rounded-2xl flex items-center justify-center overflow-hidden border-2 border-earth-700/50 shadow-md">
      ...
      <!-- Right Controls -->
      <div class="flex items-center flex-col-reverse md:flex-row gap-2 md:gap-4">
        <!-- Teleport target for Quick Scroll -->
        <div id="navbar-quick-scroll"></div>
        ...
        <div class="relative">
          <motion.button :whileTap="{ scale: 0.92 }" @click="toggleMenu"
            ...
            <span class="relative w-5 h-4 flex items-center">
              ...
              <motion.span :animate="menuOpen ? { opacity: 0, scaleX: 0 } : { opacity: 1, scaleX: 1 }"
                :transition="{ duration: 0.18 }" class="absolute w-5 h-0.5 bg-earth-700 rounded-full"
            ...
            <motion.div v-if="menuOpen" :initial="{ opacity: 0, scale: 0.95, y: -8 }"
              ...
              <div v-else class="py-2">
                <div class="px-4 py-3 border-b border-earth-50">
                  ...
                  <p class="text-earth-700 text-sm font-semibold truncate mt-0.5">{{
          ...
          <div v-if="menuOpen" class="fixed inset-0 z-[-1]" @click="closeMenu" />
        </div>
      </div>
    </div>
  </motion.header>
