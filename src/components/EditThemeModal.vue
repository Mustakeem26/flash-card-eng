    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6">
      ...
      <motion.div initial="{ opacity: 0, scale: 0.9, y: 20 }" animate="{ opacity: 1, scale: 1, y: 0 }"
        ...
        <div class="px-8 py-6 border-b border-coral-100 flex justify-between items-center bg-coral-50/50">
          <div>
            <h2 class="text-2xl font-serif text-coral-900 font-bold">Edit Collection</h2>
            <p class="text-coral-500 text-xs font-sans font-bold uppercase tracking-widest mt-1">Archive Entry
              Modification</p>
          </div>
          <button @click="close" class="text-coral-300 hover:text-coral-600 transition-colors">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div class="space-y-6">
              ...
              <div class="space-y-4 bg-coral-50/30 p-11 rounded-3xl border border-coral-300">
                ...
                <motion.button :whileHover="canAppend ? { scale: 1.02 } : {}"
                  ...
                  class="w-full bg-coral-300 text-white font-bold py-3 rounded-xl transition-colors shadow-lg shadow-coral-900/10 disabled:opacity-30 disabled:cursor-not-allowed"
                  :class="canAppend ? 'hover:bg-coral-600' : ''">
                  Append to List
                </motion.button>
              </div>
            </div>
            ...
            <div class="space-y-6">
              ...
              <div @dragover.prevent="dragActive = true" @dragleave.prevent="dragActive = false" @drop="handleDrop"
                ...
                <div
                  class="w-12 h-12 bg-white rounded-2xl shadow-sm flex items-center justify-center text-coral-300 mb-4">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
        ...
        <!-- Footer -->
        <div class="px-8 py-6 border-t border-coral-100 bg-white flex justify-between items-center">
        ...
        <AnimatePresence>
          <div v-if="showConfirmDelete" class="absolute inset-0 z-[60] flex items-center justify-center p-6">
            <motion.div initial="{ opacity: 0 }" animate="{ opacity: 1 }" exit="{ opacity: 0 }"
              @click="showConfirmDelete = false" class="absolute inset-0 bg-coral-900/20 backdrop-blur-md"></motion.div>
            <motion.div initial="{ opacity: 0, scale: 0.9, y: 20 }" animate="{ opacity: 1, scale: 1, y: 0 }"
              exit="{ opacity: 0, scale: 0.9, y: 20 }"
              class="relative bg-white p-10 rounded-[40px] shadow-2xl max-w-sm w-full text-center border border-coral-200">
              <div
              ...
              <h3 class="text-2xl font-serif text-coral-900 font-bold mb-3">Delete Collection?</h3>
              <p class="text-coral-500 text-sm mb-10 leading-relaxed">This action cannot be undone. All flashcards in
                this collection will be permanently removed.</p>
              <div class="flex flex-col gap-3">
                <button @click="deleteTheme" :disabled="isDeleting"
                  class="w-full bg-coral-300 text-white font-bold py-4 rounded-2xl hover:bg-coral-600 transition-all shadow-lg shadow-coral-500/20 disabled:opacity-50">
                  {{ isDeleting ? 'Deleting...' : 'Yes, Delete Collection' }}
                </button>
                <button @click="showConfirmDelete = false" :disabled="isDeleting"
                  class="w-full bg-coral-50 text-coral-600 font-bold py-4 rounded-2xl hover:bg-coral-100 transition-all">
                  Cancel
                </button>
              </div>
            </motion.div>
